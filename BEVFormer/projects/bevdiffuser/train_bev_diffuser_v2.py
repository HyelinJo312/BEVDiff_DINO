...
from model_utils import get_bev_model, build_unet, get_bev_model_scratch, get_bev_model_v2
...
def train():
    ...
    bev_model = get_bev_model_v2(args)
    ...
    if args.pretrained_unet_checkpoint is not None and (os.path.isfile(args.pretrained_unet_checkpoint) or os.path.isdir(args.pretrained_unet_checkpoint)):
        ...
        logger.info(f"Successfully load pretrained unet from {args.pretrained_unet_checkpoint}")
    ...
    # create custom saving & loading hooks so that `accelerator.save_state(...)` serializes in a nㅍㅇ치뮤ice format
    ...
    #     trained_params += list(bev_model.parameters()) + list(fuser.parameters())
    
    unet_params = list(unet.parameters())
    # fuser_params = list(fuser.parameters())
    bev_params = list(bev_model.parameters())
    ...
    warmup_steps_unet = args.lr_warmup_steps * accelerator.num_processes

    lambda_unet = get_diffusers_lambda(
        args.lr_scheduler,
        total_steps=total_steps,
        warmup_steps=warmup_steps_unet)
    
    lambda_bev = get_bev_cosine_warmup_lambda(
    ...
    lr_scheduler = torch.optim.lr_scheduler.LambdaLR(
        optimizer,
        lr_lambda=[lambda_unet, lambda_bev]
    )
    ...
    # unet, fuser, optimizer, lr_scheduler = accelerator.prepare(
    #     unet, fuser, optimizer, lr_scheduler
    # )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                # Feature refinement    
                # fused_bev = fuser(bev_embed, model_pred)  
                ...
                # if args.task_loss_scale > 0 and global_step > 5000 and noise_scheduler.config.prediction_type == "sample":
                #     task_loss = get_task_loss(fused_bev, **batch)
                # else:
                #     task_loss = torch.tensor(0.0, device=accelerator.device)

                # aux_task_loss = get_task_weighted_loss(bev_embed, **batch)

                total_loss = denoise_loss + args.task_loss_scale * task_loss

                # get learing rate
                lr = optimizer.param_groups[0]["lr"]
                lr_bev  = optimizer.param_groups[1]["lr"]   
                ...
                loss_dict = {
                    ...
                    # "train/aux_task_loss": aux_task_loss
                }
                ...
                #     accelerator.clip_grad_norm_(trained_params, args.max_grad_norm)
                
                torch.nn.utils.clip_grad_norm_(unet.parameters(), args.max_grad_norm)
                # torch.nn.utils.clip_grad_norm_(fuser.parameters(), args.max_grad_norm)
                torch.nn.utils.clip_grad_norm_(bev_model.parameters(), max_norm=35, norm_type=2)
